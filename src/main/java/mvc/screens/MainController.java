    ...
    public void switchView(ScreenType screenType, Object... args) {
        switch (screenType) {
            case PERSONLIST:
                ...
                personList = (ArrayList<Person>) personGateway.fetchPeople(session.getToken());
                if(args[0] instanceof Person) {
                    this.personList.add((Person) args[0]);
                }
                loader.setController(new PersonListController(personList, session));
...
//    }
//
//    public void setCatGateway(PersonGateway personGateway) {
